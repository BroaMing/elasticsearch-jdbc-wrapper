...
public class Sum extends AbstractCommonFunction {
    /**
     * コンストラクター.
     */
    public Sum() {
    ...
    private void fillResult(final List<Map<String, Object>> aggsContainer, final Map<String, Object> resultMap, final Aggregation aggregation) {
        ...
        } else if (aggregation instanceof org.elasticsearch.search.aggregations.metrics.sum.Sum) {
            org.elasticsearch.search.aggregations.metrics.sum.Sum value = 
                    (org.elasticsearch.search.aggregations.metrics.sum.Sum) aggregation;
            if (StringUtils.equals(this.getName(), value.getName())) {
                resultMap.put(value.getName(), value.getValue());
        ...
        if (this.parameters.size() > 0) {
            for (ISelectItem param : this.parameters) {
                if (param instanceof ISource) {
                    AbstractAggregationBuilder<?> aggs = AggregationBuilders.sum(this.getName()).field(param.getName());
                    if (retValue == null) {
                        retValue = aggs;
                    } else {
