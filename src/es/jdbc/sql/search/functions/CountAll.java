...
public class CountAll extends AbstractCommonFunction {
    /**
     * コンストラクター.
     */
    public CountAll() {
        this.parameters = new ArrayList<ISelectItem>();
        this.isTotalAggregation = true;
    }
    ...
    private void fillResult(final List<Map<String, Object>> aggsContainer, final Map<String, Object> resultMap, final Aggregation aggregation) {
        ...
        if (aggregation instanceof Terms) {
            ...
            if (this.isDistinct) {
                if (StringUtils.equals(this.getName(), termName)) {
                    resultMap.put(CommonParams.ELASTICSEARCH_AGGREGATION_KEYS.COLUMN_TYPE_AGGS.getKey(), this.getName());
                    resultMap.put(CommonParams.ELASTICSEARCH_AGGREGATION_KEYS.TYPE.getKey(), this.getType());
                    if (bucketList != null && bucketList.size() > 0) {
                        resultMap.put(this.getName(), bucketList.size());
                    } else {
                        resultMap.put(this.getName(), 0);
                    }
        ...
        if (this.parameters.size() > 0) {
            for (ISelectItem countParam : this.parameters) {
                if (countParam instanceof ISource) {
                    ...
                    if (this.isDistinct) {
                        countValue = AggregationBuilders.terms(this.getName()).field(countParam.getName()).size(Integer.MAX_VALUE);
                    } else {
                    ...
                    AbstractAggregationBuilder<?> funcAggression = ((IFunction) countParam).buildAggregation();
                    if (retValue == null) {
