...
public class FirstValue extends AbstractCommonFunction {
    /**
     * 取得対象カラム名称.
     */
    ...
    private void fillResult(final List<Map<String, Object>> aggsContainer, final Map<String, Object> resultMap, final Aggregation aggregation) {
        ...
        if (aggregation instanceof Terms) {
            ...
            TopHits value = (TopHits) aggregation;
            if (StringUtils.equals(this.getName(), value.getName())) {
                ...
                if (value.getHits().getTotalHits() > 0) {
                    SearchHit searchResult = value.getHits().getHits()[0];
                    Map<String, Object> valueMap = new HashMap<String, Object>();
                    for (String fieldName : this.fieldNames) {
                        valueMap.put(fieldName, searchResult.field(fieldName).getValue());
                    }
                    resultMap.put(this.getName(), valueMap);
                } else {
                    resultMap.put(this.getName(), null);
                }
        ...
        if (this.parameters.size() > 0) {
            for (ISelectItem param : this.parameters) {
                if (param instanceof ISource) {
                    ...
                    if (preBuilder != null && preBuilder instanceof TopHitsAggregationBuilder) {
                        //((TopHitsBuilder) preBuilder).addField(fieldName);
                        ((TopHitsAggregationBuilder) preBuilder).fieldDataField(fieldName);
                    } else {
                        //AbstractAggregationBuilder<?> currentValue = AggregationBuilders.topHits(this.getName()).addField(fieldName).setSize(1);
                        AbstractAggregationBuilder<?> currentValue = AggregationBuilders.topHits(this.getName()).fieldDataField(fieldName).size(1);
                        if (retValue == null) {
                    ...
                    if (param instanceof InnerOrder) {
                        if (preBuilder != null && preBuilder instanceof TopHitsAggregationBuilder) {
                            ...
                            TopHitsAggregationBuilder firstValueNode = ((TopHitsAggregationBuilder) preBuilder);
                            
                            LinkedHashMap<String, SortOrder> sortMap = innerOrder.createSortMap();
                            Iterator<String> fieldNames = sortMap.keySet().iterator();
                            while (fieldNames.hasNext()) {
                                String fieldName = fieldNames.next();
                                SortOrder sortOrder = sortMap.get(fieldName);
                                //firstValueNode.addSort(fieldName, sortOrder);
                                firstValueNode.sort(fieldName, sortOrder);
                            }
                        }
                    } else {
                        //Other Function
