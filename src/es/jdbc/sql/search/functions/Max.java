...
public class Max extends AbstractCommonFunction {
    /**
     * コンストラクター.
     */
    public Max() {
    ...
    private void fillResult(final List<Map<String, Object>> aggsContainer, final Map<String, Object> resultMap, final Aggregation aggregation) {
        ...
        } else if (aggregation instanceof org.elasticsearch.search.aggregations.metrics.max.Max) {
            org.elasticsearch.search.aggregations.metrics.max.Max value = 
                    (org.elasticsearch.search.aggregations.metrics.max.Max) aggregation;
        ...
        if (this.parameters.size() > 0) {
            for (ISelectItem param : this.parameters) {
                if (param instanceof ISource) {
                    AbstractAggregationBuilder<?> aggs = AggregationBuilders.max(this.getName()).field(param.getName());
                    if (retValue == null) {
                        retValue = aggs;
                    } else {
